...
const CMS = () => {
  ...
  const callAPI = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/woods`, {
        method: "GET",
        headers: {
      ...
      console.log("Fetch woods success:", result);
  ...
  const callPostAPI = async () => {
    try {
      ...
      const result = await response.json();
      console.log("Add wood success:", result);
      handleClose();
      callAPI()
    } catch (error) {
      console.error("Error adding wood:", error);
      alert(error.message); // Show the exact validation error in a popup!
    }
  };
  // ________________ POST API CALL ________________
  ...
  const callDeleteAPI = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE}/api/woods/` + id, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
      ...
      const result = await response.json();
      console.log("Delete wood success:", result);
      handleCloseDelete();
      callAPI()
    } catch (error) {
  ...
  const callEditAPI = async () => {
    try {
      const raw = {
        ...
        type: type || idData?.type,
        origin: origin || idData?.origin,
        color: color || idData?.color,
        density: density !== "" ? Number(density) : idData?.density,
        pricePerUnit: pricePerUnit !== "" ? Number(pricePerUnit) : idData?.pricePerUnit,
        description: description || idData?.description,
        available: available,
        image: image || idData?.image,
      };
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE}/api/woods/` + idData?._id, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
      ...
      const result = await response.json();
      console.log("Edit wood success:", result);
      handleCloseEdit();
      callAPI()
    } catch (error) {
      console.error("Error editing wood:", error);
      alert(error.message);
    }
  };
  // ________________ Edit API CALL ________________
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title className="modal-title-custom">Add Wood</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="mb-3">
            <label className="form-label-custom">Name</label>
            <input type="text" className="form-control" placeholder="e.g. Teak" onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Type</label>
            <select className="form-select" onChange={(e) => setType(e.target.value)} defaultValue="hardwood">
              <option value="hardwood">Hardwood</option>
              <option value="softwood">Softwood</option>
            </select>
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Origin</label>
            <input type="text" className="form-control" placeholder="e.g. India" onChange={(e) => setOrigin(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Color</label>
            <input type="text" className="form-control" placeholder="e.g. Golden Brown" onChange={(e) => setColor(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Density (kg/m³)</label>
            <input type="number" className="form-control" placeholder="e.g. 720" onChange={(e) => setDensity(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Price Per Unit (₹)</label>
            <input type="number" className="form-control" placeholder="e.g. 3200" onChange={(e) => setPricePerUnit(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Description</label>
            <input type="text" className="form-control" placeholder="e.g. Premium teak wood" onChange={(e) => setDescription(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Image URL</label>
            <input type="text" className="form-control" placeholder="https://..." onChange={(e) => setImage(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Available</label>
            <select className="form-select" onChange={(e) => setAvailable(e.target.value === "true")} defaultValue="true">
      ...
      <Modal show={showEdit} onHide={handleCloseEdit}>
        <Modal.Header closeButton>
          <Modal.Title className="modal-title-custom">Edit Wood</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="mb-3">
            <label className="form-label-custom">Name</label>
            <input type="text" className="form-control" defaultValue={idData?.name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Type</label>
            <select className="form-select" defaultValue={idData?.type} onChange={(e) => setType(e.target.value)}>
              <option value="hardwood">Hardwood</option>
              <option value="softwood">Softwood</option>
            </select>
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Origin</label>
            <input type="text" className="form-control" defaultValue={idData?.origin} onChange={(e) => setOrigin(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Color</label>
            <input type="text" className="form-control" defaultValue={idData?.color} onChange={(e) => setColor(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Density (kg/m³)</label>
            <input type="number" className="form-control" defaultValue={idData?.density} onChange={(e) => setDensity(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Price Per Unit (₹)</label>
            <input type="number" className="form-control" defaultValue={idData?.pricePerUnit} onChange={(e) => setPricePerUnit(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Description</label>
            <input type="text" className="form-control" defaultValue={idData?.description} onChange={(e) => setDescription(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Image URL</label>
            <input type="text" className="form-control" defaultValue={idData?.image} onChange={(e) => setImage(e.target.value)} />
          </div>
          <div className="mb-3">
            <label className="form-label-custom">Available</label>
            <select className="form-select" defaultValue={String(idData?.available)} onChange={(e) => setAvailable(e.target.value === "true")}>
              <option value="true">Yes</option>
              <option value="false">No</option>
            </select>
          </div>
